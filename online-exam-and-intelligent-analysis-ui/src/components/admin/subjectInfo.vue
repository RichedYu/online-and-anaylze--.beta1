   ...
   <div class="info">
    <!--    显示表格-->
        <el-table :data="tableData" height="440" stripe style="width: 100%">
            <el-table-column prop="subjectId" label="课程编号" sortable/>
            <el-table-column prop="subjectName" label="课程名称"/>
            <el-table-column prop="chapterCount" label="章节数量"/>
            <el-table-column prop="paperCount" label="试卷数量"/>
...
export default {
  ...
  methods: {
    list() {
      ...
      request.get(`/api/exam/subjectList/${current}/${size}/${keyword}`).then(res => {
