<template>
  <div class="application-question-create">
    <el-card class="form-card">
      <template #header>
        <div class="card-header">
          <span class="card-title">应用题创编</span>
        </div>
      </template>
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        ...
        <el-form-item label="题目内容" prop="question">
          ...
          <div v-if="form.questionType === 'image' && form.question.length > 0" class="image-link">
            已上传图片链接：{{ form.question }}
          </div>
        </el-form-item>
        ...
        <!-- 参考答案 -->
        <el-form-item label="参考答案" prop="correctAnswers">
          <el-input
...
export default {
  setup() {
    ...
    const form = ref({
      ...
      questionType: 'text', // 题目内容类型：text 或 image
      question: '', // 题目内容（文本或图片链接数组）
      analysis: '', // 题目解析
      correctAnswers: '', // 参考答案
      level: '', // 难度
    });
    ...
    const validateQuestion = (rule, value, callback) => {
      if (form.value.questionType === 'text' && !form.value.question) {
        callback(new Error('请输入题目内容'));
      } else if (form.value.questionType === 'image' && form.value.question.length === 0) {
        callback(new Error('请上传题目图片'));
      } else {
        callback();
      }
    };
    ...
    const handleBeforeUpload = async (file) => {
      ...
      try {
        const res = await request.post('/api/exam/file/upload', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        if (res.code === 200) {
          // 将图片链接添加到 question 数组中
          if (form.value.questionType === 'image') {
            if (!Array.isArray(form.value.question)) {
              form.value.question = [];
            }
            form.value.question.push(res.data.url);
          }
    ...
    const submitForm = () => {
      formRef.value.validate((valid) => {
        if (valid) {
          ...
          // 如果是图片题目，则将图片链接数组转为字符串
          if (Array.isArray(form.value.question)) {
            form.value.question = '[' +  form.value.question.join(',') + ']';
          }
          request.post('/api/exam/createApplication', form.value).then(
            (res) => {
              if (res.code === 200) {
