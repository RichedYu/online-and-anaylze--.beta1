<template>
    <div class="fill-blank-create">
      <el-card class="form-card">
        <template #header>
          <div class="card-header">
            <span class="card-title">填空题创编</span>
          </div>
        </template>
        ...
        <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
          ...
          <el-form-item label="题目内容" prop="question">
            <el-input
              ...
              type="textarea"
              :rows="4"
              placeholder="请输入题目内容，填空处用 {{}} 表示，例如：中国的首都是 {{}}。"
            ></el-input>
          </el-form-item>
          ...
          <!-- 填空项参考答案 -->
          <el-form-item
            v-for="(blank, index) in form.blanks"
            :key="index"
            :label="`填空项 ${index + 1}`"
  ...
  export default {
    setup() {
      ...
      watch(
        () => form.value.question,
        (newQuestion) => {
          const blankCount = (newQuestion.match(/\{\{\}\}/g) || []).length;
          form.value.blanks = Array.from({ length: blankCount }, () => ({ answer: '' }));
        }
      );
    ...
    const submitForm = () => {
      formRef.value.validate((valid) => {
        if (valid) {
          ...
          // 利用正则表达式将题目内容中{{data}}里的答案data取出，并放入correctAnswer字段用','分隔
          form.value.correctAnswers = form.value.question.match(/\{\{(.*?)\}\}/g).map(item => item.replace(/[\{\}]/g,'')).join(',');
          // 把题目内容中的{{data}}替换为____
          form.value.question = form.value.question.replace(/\{\{(.*?)\}\}/g, '____');
          console.log('题目内容:', form.value.question);
          console.log('答案:',form.value.correctAnswers);
          request.post('/api/exam/createFill', form.value).then(
            (res) => {
              if(res.code === 200) {
