<template>
    <div class="weakness-analysis-page">
      ...
      <el-card class="list-card">
        <el-table :data="weaknessList" border stripe>
          ...
          <el-table-column prop="questionCount" label="班级累计答题数" />
          <el-table-column prop="wrongCount" label="班级错题数" />
          <el-table-column prop="scoreRate" label="班级平均得分率">
            <template #default="{ row }">
              {{ (row.scoreRate * 100).toFixed(2) }}%
            </template>
          </el-table-column>
          <el-table-column prop="wrongRate" label="班级平均错题率">
            <template #default="{ row }">
  ...
  const handleClassChange = async (classId) => {
    try {
      const res = await request.get(`/api/analysis/getClassWeakSubjectKnowledgePoint/${selectedCourse.value}/${classId}`);
      if (res.code === 200) {
        weaknessList.value = res.data;
        renderChart(res.data);
      } else {
        ElMessage.error('获取班级薄弱知识点数据失败');
      }
    } catch (error) {
      console.error('获取班级薄弱知识点数据失败:', error);
      ElMessage.error('获取班级薄弱知识点数据失败');
    }
  };
