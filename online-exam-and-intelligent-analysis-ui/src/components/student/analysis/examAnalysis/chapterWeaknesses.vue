...
import Schart5Vue from '@/components/charts/Schart5.vue';
<template>
    <div>
        <el-row class="card1-part">
            ...
            <div style="height: 500px;width: 100%;"><Schart5/></div>
            <el-divider border-style="double" />
            <div style="height: 500px;width: 100%;"><Schart6/></div>
            <el-divider border-style="double" />
            <div style="text-align: center;text-align: left;">
                <el-button type="primary" style="margin-left: 3px;" @Click='getAnalysis3'>点击生成ai分析报告</el-button>
  ...
  'Hiragino Sans GB', 'Microsoft YaHei', '微软雅黑', Arial, sans-serif;line-height: 26px;font-size: 16px;background-color: #f0f2f5;padding: 10px;border-radius: 5px;color: #666;margin: 10px 10px;height:300px;width:900px;padding: 10px 10px;">
                <span style="margin:0px;">{{analysis3}}</span>
                </div>
            </div>
    ...
    methods: {
      ...
      getAnalysis3() {
        ...
        request.get(`/api/login/examAnalysis4/${this.analysisScoreId}`).then((res) => {
          if(res.code === 200){
            console.log(res.data)
            this.analysis3 = res.data.message
            this.loading = false
          }else{
            alert("请求失败")
            // this.getAnalysis3()
